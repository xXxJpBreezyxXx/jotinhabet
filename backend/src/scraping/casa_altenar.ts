...
interface AltResp { events?: AltEvent[]; markets?: AltMarket[]; odds?: AltOdd[]; competitors?: AltCompetitor[]; champs?: any[]; }
...
export class AltenarWidgetScraper implements OddsScraper {
  ...
  private async extrairEsporte(sportId: number): Promise<ScrapedOdd[]> {
    // 1) Campeonatos com jogos, maiores primeiro.
    const menuResp = await fetchTextoComRetry(
      `${this.F}/widget/GetClickableSportMenu?${this.q()}&sportId=${sportId}`,
      { headers: this.headers() }, 3, `${this.cfg.nome}/menu`
    );
    if (menuResp.status !== 200) throw new Error(`menu HTTP ${menuResp.status}`);
    const menu: AltResp = JSON.parse(menuResp.body);
    const champs = (menu.champs || [])
      .filter((c: any) => c.eventsCount > 0)
      .sort((a: any, b: any) => (b.eventsCount || 0) - (a.eventsCount || 0))
      .slice(0, this.cfg.maxCampeonatosPorEsporte);

    const odds: ScrapedOdd[] = [];
    const vistos = new Set<number>(); // evita reprocessar o mesmo evento entre champs
    ...
    for (let i = 0; i < champs.length; i += 5) {
      ...
      this.parseResposta(j, sportId, odds, vistos);
    }
    return odds;
  }

  private parseResposta(j: AltResp, sportId: number, out: ScrapedOdd[], vistos: Set<number>): void {
    ...
    for (const ev of j.events || []) {
      ...
      for (const mid of ev.marketIds || []) {
        ...
        // --- Total (Over/Under), linha em sv ---
        else if (m.name === 'Total' && m.sv) {
          ...
          out.push({
            esporte, evento, dataHora, mercado: TOTAL_LABEL[sportId] || 'Total',
